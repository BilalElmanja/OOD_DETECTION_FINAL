...
class OpenImage_O(Dataset):
    def __init__(self, train=False, transform=None):
        ...
        """
        self.root_dir = "../../data/OpenImage-O"
        self.train = train
...
transform = transforms.Compose([
    # ResNet models expect 3-channel images, but OpenImage_O is already in this format
    ...
    # Normalize each channel of the OpenImage_O images using mean and std
    # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    # torchvision.models.ResNet50_Weights.IMAGENET1K_V1.transforms()
 
])

def get_train_dataset_OpenImage_O():
    # Initialize the SSB_hard datasets for training and testing
    train_dataset = OpenImage_O( train=True, transform=transform)
    print("the length of the OpenImage_O Training dataset : ", len(train_dataset))
    # Create the DataLoaders for training and testing
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=4)
    return train_loader

def get_test_dataset_OpenImage_O():
    # Initialize the inaturalist datasets for training and testing
    test_dataset = OpenImage_O( train=False, transform=transform)
    print("the length of the OpenImage_O Test dataset : ", len(test_dataset))
    # Create the DataLoaders for training and testing
