...
from methods_v2 import K_Means, NMF_KNN, PCA_KNN, NMF_CONCEPTS, CRAFT_CONCEPTS, PCA_NORM, NMF_CONCEPTS_VIZ, NMF_CONCEPTS_VIZ_ALL_CLASSES, NMF_CONCEPTS_VIZ_SCORING_WTHOUT_PATCHES
from data_preprocessing import get_train_dataset_cifar10, get_test_dataset_cifar10, get_train_dataset_cifar100, get_test_dataset_cifar100, get_test_dataset_places365, get_test_dataset_svhn, get_test_dataset_texture, get_test_dataset_Tiny, get_test_dataset_NINCO, get_test_dataset_OpenImage_O, get_test_dataset_inaturalist, get_test_dataset_SSB_hard, get_test_dataset_ImageNet_1K_OOD, get_train_dataset_ImageNet_1K, get_test_dataset_ImageNet_1K, get_train_dataset_Tiny
...
def load_datasets(experiment="imagenet"):
    ...
    if experiment == "imagenet":
        ...
        ds_out_dict = {
            ...
            "SSB-HARD" : get_test_dataset_SSB_hard(),
            "Texture" : get_test_dataset_texture(),
            "openImage-O": get_test_dataset_OpenImage_O(),
            "NINCO": get_test_dataset_NINCO(),
            "Imagenet-OOD": get_test_dataset_ImageNet_1K_OOD()
        }

    elif experiment == "imagenet-200":
        ds_fit = get_train_dataset_Tiny()
        ds_in = get_test_dataset_Tiny()
        ds_out_dict = {
            "Imagenet" : get_test_dataset_ImageNet_1K(),
            "INaturalist": get_test_dataset_inaturalist(),
            "Places365" : get_test_dataset_places365(),
            "SSB-HARD" : get_test_dataset_SSB_hard(),
            "Texture" : get_test_dataset_texture(),
        ...
        raise ValueError("`experiment` should be only 'imagenet'")
...
class BenchmarkTorch:
    ...
    DETECTORS_CONFIG = {
        ...
        "NMF_CONCEPT_FINAL": {
            "class": NMF_CONCEPTS_VIZ,
            "kwargs": {
                "imagenet": dict( model=model, separate_classes=[0, 1, 2, 3,]),
            },
            "fit_kwargs": {
                "imagenet": dict(feature_layers_id=["avgpool"]),
            },
        },

        "NMF_CONCEPT_FINAL_SCORING_WTHOUT_PATCHES": {
            "class": NMF_CONCEPTS_VIZ_SCORING_WTHOUT_PATCHES,
            "kwargs": {
                "imagenet": dict( model=model, separate_classes=[0, 1, 2, 3]),
            },
            "fit_kwargs": {
        ...
        "NMF_CONCEPT_FINAL_ALL_CLASSES": {
            "class": NMF_CONCEPTS_VIZ_ALL_CLASSES,
            "kwargs": {
                "imagenet": dict( model=model, ),
            },
            "fit_kwargs": {
                "imagenet": dict(feature_layers_id=["avgpool"]),
            },
    ...
    def _benchmark_one_experiment(self, experiment: str):
        ...
        # datasets
        ds_fit, ds_in, ds_out_dict = load_datasets(experiment)
...
if __name__ == "__main__":
    dir_path = os.path.expanduser("~/") + "./results/FINAL_IMAGENET_BENCHMARK_ALL_WITHOUT_CONTINUE_2"
    os.makedirs(dir_path, exist_ok=True)
