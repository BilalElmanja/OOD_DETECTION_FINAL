...
parser.add_argument(
    "--cuda", type=int, default=1, help="Index of the CUDA device to use (default: 0)"
)
parser.add_argument(
...
# models
def load_model(experiment="imagenet", load_mlp=False):
...
# datasets
def load_datasets(experiment: str = "imagenet", batch_size: int = 128):
...
class BenchmarkTorch:
    ...
    DETECTORS_CONFIG = {
        ...
        "CRAFT_NMF_16_10_0.8": {
            ...
            "kwargs": {
                "cifar10": dict( n_components=10, n_important_concepts=10, percentage_images_concept=0.8, model=load_pretrained_weights_32()),
                "cifar100": dict( n_components=16, n_designs=16, model=load_pretrained_weights_32()),
                "imagenet": dict( n_components=64, n_important_concepts=64, percentage_images_concept=0.8, model=nn.Sequential(*(list(load_model(experiment="imagenet").children())[-1:]))),
...
if __name__ == "__main__":
    dir_path = os.path.expanduser("~/") + "./results/imagenet_craft_concepts"
    os.makedirs(dir_path, exist_ok=True)
