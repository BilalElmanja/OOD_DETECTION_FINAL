...
class NMF_CONCEPTS_VIZ_ALL_CLASSES(OODBaseDetector):
    def __init__(self, model=None, n_components=30, n_neighbors=20, percentage_of_images=0.1, distance="euclidean", separate_classes=[6], crop_size=64, slide_size=6):
        super().__init__()
    ...
    def images_to_patches(self):
        ...
        for image, label in self.original_dataset:
            ...
            all_patches.extend([(patch, label) for patch in patches])
            # Replace original images with their patches
        self.images_patches = all_patches
        print("Number of patches: ", len(self.images_patches))
    ...
    def _fit_to_dataset(self, fit_dataset):
        ...
        print("The length of the dataset: ", self.dataset_length)
        self.original_dataset = fit_dataset.dataset
        ...
        images_fit = [(self.preprocess(image), label) for image, label in self.images_patches]
        self._process_dataset(images_fit)

        # self.show_concepts_per_class(self.separate_classes[0])

    def _process_dataset(self, images):
        ...
        for concept in range(self.n_components):
            ...
            original_patches = [self.images_patches[i][0] for i in sorting_indices]
        ...
        # for concept in range(self.n_components):
        #     patches = self.nmf_models['top_activated_images'][concept]
        #     self.visualize_patches(patches, f"./all_concepts/concept_{concept}")
    ...
    def process_and_visualize_new_image(self, image):
        patches = self._extract_patches(image)
        print("Number of patches: ", len(patches))

        # Get the NMF model for the class
        nmf_data = self.nmf_models
        nmf = nmf_data['nmf']
        ...
        # plt.axis('off')
        plt.show()
        plt.savefig(f"new_image_heatmap.png")

    def show_concepts_per_class(self):
        # get an original image from the class
        # choose a random integer between 0 and 300
        random_index = np.random.randint(0, 100)
        original_image = self.original_dataset[random_index]
        self.process_and_visualize_new_image(original_image)
        # show patches of the class
        # for concept in range(self.n_components):
        #     patches = self.nmf_models['top_activated_images'][concept]
        #     self.visualize_patches(patches, f"_concept_{concept}")
    ...
    def _score_tensor(self, inputs):
        ...
        nmf = self.nmf_models['nmf']
        U = nmf.transform(patches_features) # shape (49, n_components)
        ...
        for concept in self.nmf_models['most_important_concepts']:
            top_activated_images_features = self.nmf_models['top_activated_images_features'][concept]
            ...
            patches_distances = []
            for i in range(49):
                distance = np.linalg.norm(U[i] - top_activated_images_features, axis=1)
...
# detector = NMF_CONCEPTS_VIZ_2(model=model)
