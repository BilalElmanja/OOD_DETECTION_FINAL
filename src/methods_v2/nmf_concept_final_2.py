...
# main class
class NMF_CONCEPTS_VIZ_SCORING_WTHOUT_PATCHES(OODBaseDetector):
    ...
    def _score_tensor(self, inputs):
        ...
        loader = DataLoader(self.preprocess(inputs), batch_size=1, shuffle=False)
        features = self.feature_extractor.predict(loader)[0]
        # convert to numpy and reshape
        features = self.op.convert_to_numpy(features[0])
        features = features.reshape(features.shape[0], -1)
        ...
        for label, nmf_data in self.nmf_models.items():
            nmf = nmf_data['nmf']
            U[label] = nmf.transform(features) # shape (49, n_components)
            ...
            for concept in nmf_data['most_important_concepts'][:5]:
                ...
                patches_distances = []
                distance = np.linalg.norm(U[label] - top_activated_images_features, axis=1)
                patches_distances.append(distance)
                
                patches_distances = np.array(patches_distances)
                # print("shape of distances: ", patches_distances.shape) # --> shape (1, 10)
                mean_distance = np.mean(patches_distances, axis=1)
                # print("shape of mean_distances: ", mean_distances.shape)
                concept_patches_distances[concept] = mean_distance
